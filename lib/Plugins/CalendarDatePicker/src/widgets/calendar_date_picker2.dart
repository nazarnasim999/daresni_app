...
import '/Plugins/CalendarDatePicker/src/utils/date_util.dart';
                ...
                child: InkWell(
                  ...
                  child: Padding(
                    padding: EdgeInsets.symmetric(
                        horizontal: widget.config.centerAlignModePicker == true ? 0 : 8),
                    child: Row(
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  DateTime? _nextDateInDirection(DateTime date, TraversalDirection direction) {
    ...
    while (
        !nextDate.isBefore(widget.config.firstDate) && !nextDate.isAfter(widget.config.lastDate)) {
...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    //
    // Color highlightDays(int day){
    //
    //
    //    bookingController.bookingAvailableDays?.data?.forEach((element) {
    //
    //     DateTime parsedDate = DateTime.parse(element.date??'');
    //    if( parsedDate.day==day) {
    //
    //      if(element.status=="Available") return AppTheme.secondaryColor;
    //
    //
    //
    //     }
                ...
                children: [
                  ...
                  if (day >= DateTime.now().day)
                    Container(
                      ...
                      width: 5,
                      decoration:
                          BoxDecoration(color: widget.indicatorColor, shape: BoxShape.circle),
                    )
                ],
              ),
            ),
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    final bool isDisabled =
        year < widget.config.firstDate.year || year > widget.config.lastDate.year;
    ...
    TextStyle? itemStyle =
        widget.config.yearTextStyle ?? textTheme.bodyLarge?.apply(color: textColor);
    if (isSelected) {
