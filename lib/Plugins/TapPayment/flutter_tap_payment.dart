...
import 'dart:core';
import 'package:darasni/Plugins/TapPayment/src/screens/complete_payment.dart';
...
class TapPaymentState extends State<TapPayment> {
  ...
  loadPayment() async {
    ...
    try {
      var getPayment = await services.sendPayment();
  ...
  void initState() {
    ...
    services =
        TapServices(apiKey: widget.apiKey, paymentData: {...widget.paymentData, ...formData});
    setState(() {
  ...
  bool canPop()  {
    if (pressed < 2) {
      ...
      // final snackBar =
      //     SnackBar(content: Text('Press back ${3 - pressed} more times to cancel transaction'));
